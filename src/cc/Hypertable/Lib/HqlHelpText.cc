...
namespace {
  ...
  const char *help_text_create_table[] = {
    ...
    "are kept stored in reverse chronological order of timestamp and the MAX_VERSIONS",
    "allows you to specify that you only want to keep the n most recent versions of",
    "each cell.  Older versions are lazily garbage collected through the normal",
    "compaction process.  If TIME_ORDER is DESC (descending) then the Cells are kept",
  ...
  const char *help_text_load_data_infile[] = {
    ...
    "during the execution of this command may result in data loss",
    "",
    "NO_ESCAPE",
    "",
